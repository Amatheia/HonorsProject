...
?>
    ...
    <section>
      <article>
        <br />
          <h2>Online Articles/Creative Writing Submissions</h2>
        <br />  
      </article>
    </section>
        ...
        <div id="requirements">
          <p>Please read all requirements prior to submitting. All submissions are 
           ...
           <br />
            ...
            <p>You are the copyright holder to the written work. The written 
            work includes a title, is less than 2,500 words, and a Word doc file. 
            If the article includes images, combine 
            article and images into a single ZIP file (less than 20MB). 
            Image(s) should be 1200 pixels on its longest side; at 72dpi, in the 
            RGB color space. Soul Efex has a no-tolerance policy for plagiarism. 
            Writing may be edited and we reserve the right to publish our edited 
            version without your prior approval.</p>
        <br />
        </div>
      </article>
    </section>
            ...
            && ($zipfile_size > 0) && ($zipfile_size <= SE_MAXFILESIZE)) {
                if ($_FILES['zipfile']['error'] == 0) {
                    ...
                    if (move_uploaded_file($_FILES['zipfile']['tmp_name'], $target)) {
                        ...
                        $query5 = "INSERT INTO online_writing_submissions (online_submit_id, title) " .
                                  "VALUES ('$id4', '$title')";
  ...
  <div class="formrow">
    <form enctype="multipart/form-data" id="submit" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
      <input type="hidden" name="MAX_FILE_SIZE" value="<?php echo SE_MAXFILESIZE; ?>" />
        ...
        <label for="zipfile">Submission DOC or ZIP File:</label>
